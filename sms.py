...
def toplevel_data(title, button_text, command):
    ...
    idLabel = ttk.Label(screen, text='Id', font=('times new roman', 20, 'bold'))
...
def connect_database():
    def connect():
        ...
        try:
            ...
            return
        try:
            query='create database studentgateway'
            mycursor.execute(query)
            query='use studentgateway'
            mycursor.execute(query)
...
root.geometry('1174x700+50+20')
root.resizable(False, False)
root.title('Student Gateway')

datetimeLabel=Label(root, font=('times new roman', 18, 'bold'))
datetimeLabel.place(x=5, y=5)
clock()
s='Student Gateway' #s[count]=t when count is 1
sliderLabel=Label(root, font=('arial', 28, 'italic bold'), width=20)
sliderLabel.place(x=400, y=0)
slider()

connectButton=ttk.Button(root, text='Connect Database', command=connect_database)
connectButton.place(x=980, y=1)
...
rightFrame.place(x=350, y=88, width=820, height=600)
